package com.astrivix.qr114.ui.widget.connect;
...
import com.astrivix.qr114.data.adapter.ScanDeviceAdapter;
import com.astrivix.qr114.data.model.basic.StateResult;
import com.astrivix.qr114.data.model.device.ScanBtDevice;
import com.astrivix.qr114.databinding.DialogScanDeviceBinding;
import com.astrivix.qr114.tool.bluetooth.rcsp.DeviceOpHandler;
import com.astrivix.qr114.ui.widget.DevicePopDialog.DevicePopDialogFilter;
...
public class ScanDeviceDialogModify extends DialogFragment implements DevicePopDialogFilter.IgnoreFilter {
    ...
    private void addObserver() {
        ...
        // =================================================================================
        //  CORRECTED OBSERVER LOGIC SCAN DEVICES SASA
        // =================================================================================
        viewModel.scanStateMLD.observe(getViewLifecycleOwner(), stateResult -> {
            ...
            switch (stateResult.getState()) {
                ...
                case StateResult.STATE_WORKING: {
                    // Get the SINGLE device object from the result.
                    ScanBtDevice foundDevice = stateResult.getData();

                    // The original code used a null payload to clear the list at the start of a scan. We must keep this behavior.
                    if (foundDevice == null) {
                        mAdapter.setList(new ArrayList<>());
                        return;
                    }

                    // --- CORRECTED FILTERING LOGIC FOR A SINGLE DEVICE ---

                    // We now have one device. Check if its address matches our target.
                    if (foundDevice.getDevice() != null && sessionManager.getKeySerialNumber().equalsIgnoreCase(foundDevice.getDevice().getAddress())) {

                        // It's a match! Add this one device to the adapter's list.
                        mAdapter.addData(foundDevice);
                        Log.i("sasa", "MATCH FOUND! Adding device to list: " + foundDevice.getDevice().getAddress());
                       String message = "MATCH FOUND! Adding device to list: " + foundDevice.getDevice().getAddress();

                    //    Toast.makeText(MainApplication.getApplication().getBaseContext(),message, Toast.LENGTH_LONG).show();
                    } else {
                        // The device did not match, so we simply ignore it and do nothing.
                        if (foundDevice.getDevice() != null) {
                            Log.v("sasa", "Ignoring device (address does not match): " + foundDevice.getDevice().getAddress());
//                            String message = "Ignoring device (address does not match): " + foundDevice.getDevice().getAddress();
//
//                            Toast.makeText(MainApplication.getApplication().getBaseContext(),message, Toast.LENGTH_LONG).show();
