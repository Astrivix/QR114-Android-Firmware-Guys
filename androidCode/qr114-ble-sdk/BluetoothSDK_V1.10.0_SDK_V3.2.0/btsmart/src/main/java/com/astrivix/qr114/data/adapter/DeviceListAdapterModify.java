...
//        viewHolder.setVisible(R.id.iv_device_list_remove_history, isEditMode && historyDevice.getState() == HistoryDevice.STATE_DISCONNECT);
        ...
        // Show the remove icon if the device is disconnected.
        boolean isDeviceDisconnected = historyDevice.getState() == HistoryDevice.STATE_DISCONNECT;
        viewHolder.setVisible(R.id.iv_device_list_remove_history, isDeviceDisconnected);
    }
    ...
    private void updateDeviceMsgUI(ImageView ivImg, TextView tvName, String url, int failedRes, String deviceName, int endRes) {
        ...
        Log.i("sasa", "updateDeviceMsgUI: " + url + "--" + failedRes + "--" + deviceName + "--" + endRes);
